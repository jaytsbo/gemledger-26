...
### 步驟 5：初次授權與建立工作表
1. 回到 Google 試算表並重新整理網頁。
2. 頂部選單會出現 **「📊 記帳助理」** 選單。
3. 點選 **「📊 記帳助理」 > 「📋 匯入記帳範本資料庫（60 筆範例）」**（或「⚙️ 初始化自訂設定工作表」）。
4. 依系統提示完成 Google 帳號安全性授權。
5. 試算表將自動建立「**記帳資料**」與「**系統設定**」兩個分頁，並填入完整的 60 筆範例資料。

### 步驟 6：發布網頁應用程式 (Web App)
